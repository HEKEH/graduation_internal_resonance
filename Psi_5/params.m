...
Ups_ck1 = -((H*lambda^2*C_m(1)*(C_m(1)*(2*sin(omega_m)+omega_m*(cos(omega_m)*(-1+2*H*tan(theta))+(1+2*H*tan(theta))*Gamma1_m))+C_m(2)*(2-2*cos(omega_m)+omega_m*(sin(omega_m)*(-1+2*H*tan(theta))+(1+2*H*tan(theta))*Gamma2_m))))/(2*omega_m));

Ups_ck2 = -((H*lambda^2*C_m(2)*(C_m(1)*(2*sin(omega_m)+omega_m*(cos(omega_m)*(-1+2*H*tan(theta))+(1+2*H*tan(theta))*Gamma1_m))+C_m(2)*(2-2*cos(omega_m)+omega_m*(sin(omega_m)*(-1+2*H*tan(theta))+(1+2*H*tan(theta))*Gamma2_m))))/(2*omega_m));

temp1 = -((2*sin(omega_m)+cos(omega_m)*omega_m*(-1+2*H*tan(theta)))/(omega_m*(1+2*H*tan(theta))));

temp2 = (2*(-1+cos(omega_m))-sin(omega_m)*omega_m*(-1+2*H*tan(theta)))/(omega_m*(1+2*H*tan(theta)));

temp3 = (H*omega_m*((sin(2*omega_m)-2*omega_m)*C_m(1)^2+4*sin(omega_m)^2*C_m(1)*C_m(2)-(sin(2*omega_m)+2*omega_m)*C_m(2)^2))/(2+4*H*tan(theta));
